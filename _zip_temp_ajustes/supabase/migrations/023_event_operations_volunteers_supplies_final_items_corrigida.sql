...
-- Compatibilidade: algumas bases têm average_unit_cost no cardápio de vendas;
-- a coluna suggested_unit_cost não existe no schema atual.
alter table if exists public.event_sales_menu_items
  add column if not exists average_unit_cost numeric(12,2) not null default 0;
...
select
  ...
  s.unit_label,
  s.price,
  coalesce(s.average_unit_cost, 0),
