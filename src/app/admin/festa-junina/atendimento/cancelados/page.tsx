...
import { RotateCcw, Ban, Trash2, Utensils } from "lucide-react";
...
type PageProps = { searchParams?: Promise<{ restaurado?: string; excluido?: string; erro?: string }> };

function shortId(id: string) {
  return id.slice(0, 8).toUpperCase();
}

function sameEventDay(cancelledAt: string | null | undefined, eventDate: string | null | undefined) {
  if (!eventDate) return true;
  if (!cancelledAt) return false;
  return cancelledAt.slice(0, 10) === eventDate.slice(0, 10);
}
...
export default async function CancelledConsumptionOrdersPage({ searchParams }: PageProps) {
  ...
  const orders = allCancelledOrders.filter((order) => sameEventDay(order.cancelled_at, event.event_date));
  const hiddenTestOrders = allCancelledOrders.length - orders.length;
      ...
      <section className="mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="rounded-[2rem] border border-green-100 bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 max-w-4xl text-stone-700">
            Esta tela mostra somente os pedidos cancelados no dia da festa{event.event_date ? ` (${new Date(`${event.event_date}T00:00:00`).toLocaleDateString("pt-BR")})` : ""}. Cancelamentos de teste anteriores ficam ocultos e podem ser removidos definitivamente pela opção abaixo.
          </p>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            <div className="rounded-3xl bg-red-50 p-5"><p className="text-sm font-bold text-stone-600">Pedidos cancelados no dia</p><p className="mt-2 text-3xl font-black text-red-900">{orders.length}</p></div>
            <div className="rounded-3xl bg-amber-50 p-5"><p className="text-sm font-bold text-stone-600">Valor cancelado no dia</p><p className="mt-2 text-3xl font-black text-green-950">{formatCurrency(totalCancelled)}</p></div>
          ...
          {hiddenTestOrders > 0 ? (
            <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-900">
              {hiddenTestOrders} pedido(s) cancelado(s) fora do dia da festa estão ocultos nesta visão. Para limpar testes, use “Excluir definitivamente” nos registros que aparecerem quando acessados antes do filtro ou rode o SQL opcional enviado no pacote.
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-8 grid gap-4">
          ...
          {orders.length === 0 ? <div className="rounded-[2rem] border border-dashed border-green-200 bg-green-50 p-8 text-center text-sm text-stone-700">Nenhum pedido cancelado no dia da festa.</div> : null}
        </div>
      </section>
    </AdminPageShell>
  );
