...
export default async function AdminLoginPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-amber-50 text-stone-900">
      <SiteHeader />
      <section className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center justify-center px-5 py-12">
        <div className="w-full rounded-[2rem] border border-green-100 bg-white/95 p-7 shadow-2xl backdrop-blur">
          <div className="mb-6 flex flex-col items-center text-center">
            <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-white shadow">
              <Image src="/images/logo-tucxa.jpg" alt="Logo Tucxa" width={64} height={64} className="rounded-full object-contain" priority />
            </div>
            ...
            <p className="mt-2 text-sm text-stone-600">Entre com um usuário real do Supabase Auth autorizado em admin_profiles.</p>
          </div>

          {erro ? <div className="mb-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{erro}</div> : null}
