...
function selectedRow(rows: ServiceResponsibleRow[], key?: string) {
  ...
  return (
    rows.find((row) => row.id === key || row.responsibleName === key) ?? rows[0]
  );
}
...
function paymentMethodButton(
  ...
  return (
    <form action={registerCashierGroupPayment} key={method}>
      ...
      <button className="rounded-2xl border border-green-100 bg-white px-4 py-3 text-sm font-black text-green-950 shadow-sm hover:bg-green-50">
        Registrar {label}
      </button>
    </form>
  );
      ...
      <section className="mx-auto max-w-7xl px-5 py-8">
        ...
        <div className="rounded-[2rem] border border-green-100 bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 max-w-4xl text-stone-700">
            Busque o responsável, abra o fechamento, confira itens e registre o
            pagamento. O Pix do fechamento já sai com o valor total e a chave do
            Tucxa.
          </p>
        </div>
                ...
                {pending > 0 ? (
                  <div className="mt-5 rounded-3xl border border-green-100 bg-green-50 p-4">
                    ...
                    <p className="mt-1 text-sm text-stone-700">
                      ...
                      Pix do Tucxa e o valor total pendente.
                    </p>
                    <PixPaymentBox
                      ...
                      {current.orders.map((order) => {
                        ...
                        return (
                          <tr key={order.id}>
                            ...
                            <td
                              className={`p-3 font-black ${paid ? "text-green-800" : "text-red-800"}`}
                            >
                              {paid ? "Pago" : "Pendente"}
                            </td>
                          </tr>
