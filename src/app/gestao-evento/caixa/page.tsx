...
export default async function CaixaPublicPage() {
    ...
    <main className="min-h-screen bg-amber-50 text-green-950">
      ...
      <section className="mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="rounded-[2rem] border border-green-100 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-4 text-3xl font-black">Fechamento de pedidos, mesas e pagamentos</h1>
          <p className="mt-3 max-w-4xl text-stone-700">Acompanhe pedidos por mesa/responsável, itens consumidos, entrega, status de pagamento e comprovantes registrados.</p>
        ...
        <div className="mt-8 rounded-[2rem] border border-green-100 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="text-2xl font-black">Clientes, mesas e pedidos</h2><p className="mt-1 text-sm text-stone-600">Agrupamento por mesa ou responsável para facilitar conferência no caixa.</p></div>
          ...
          <div className="mt-5 grid gap-5">
              ...
              return (
                <section key={groupName} className="rounded-3xl border border-green-100 bg-stone-50 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div><p className="text-xs font-black uppercase tracking-[0.16em] text-green-800">Mesa/responsável</p><h3 className="mt-1 text-xl font-black">{groupName}</h3><p className="mt-1 text-sm text-stone-600">{groupOrders.length} pedido(s)</p></div>
                    ...
                    {groupOrders.map((order) => (
                      <article key={order.id} className="rounded-2xl bg-white p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div><p className="font-black">Pedido {shortId(order.id)} · {order.customer_name || "Cliente"}</p><p className="text-sm text-stone-600">WhatsApp: {order.customer_phone || "não informado"}</p></div>
                        ...
                        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto] md:items-center"><div className="rounded-2xl bg-stone-50 p-3 text-sm"><p className="font-black"><CreditCard className="mr-1 inline h-4 w-4" /> Pagamentos registrados</p>{order.payments.length > 0 ? <ul className="mt-2 space-y-1 text-stone-700">{order.payments.map((payment) => <li key={payment.id}>{paymentMethodLabel(payment.method)} · {formatCurrency(payment.amount)} · {paymentStatusLabel(payment.status)}{payment.proof_file_path ? " · comprovante anexado" : ""}</li>)}</ul> : <p className="mt-2 text-stone-600">Nenhum pagamento registrado.</p>}</div><Link href={`/cardapio/arraia-tucxa-2026/pedido/${order.id}`} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-green-900 px-5 py-3 text-sm font-black text-white" prefetch={false}><ReceiptText className="h-4 w-4" /> Abrir pedido</Link></div>
                      </article>
                    ))}
                  </div>
                </section>
            ...
            {orders.length === 0 ? <div className="rounded-3xl border border-dashed border-green-200 bg-green-50 p-8 text-center text-sm text-stone-700">Nenhum pedido de consumo foi registrado ainda.</div> : null}
          </div>
        </div>
      </section>
    </main>
