...
function totalFromQuantities(
  items: PublicSalesMenuItem[],
  quantities: Record<string, number>,
) {
  return items.reduce(
    (sum, item) => sum + (quantities[item.id] ?? 0) * numberValue(item.price),
    0,
  );
}
...
export function PublicSalesMenu({
  eventSlug,
  items,
  defaultServiceSessionId = "",
  defaultTableLabel = "",
  defaultCustomerName = "",
  defaultWaiterName = "",
  ...
  const [session, setSession] = useState<CustomerSession>({
    orderMode: defaultTableLabel ? "table" : "individual",
    customerName: defaultCustomerName,
    customerPhone: "",
    tableLabel: defaultTableLabel,
    waiterName: defaultWaiterName,
    settlementMode: defaultSettlementMode || "por_pedido",
  });
    ...
    const timer = window.setTimeout(() => {
      try {
        const savedSession = window.localStorage.getItem(
          getSessionKey(eventSlug),
        );
        if (savedSession) {
          const parsed = JSON.parse(savedSession) as CustomerSession;
          setSession({
            orderMode: defaultTableLabel
              ? "table"
              : parsed.orderMode || "individual",
            ...
            settlementMode:
              defaultSettlementMode || parsed.settlementMode || "por_pedido",
          });
        }
        const savedLastOrder = window.localStorage.getItem(
          getLastOrderKey(eventSlug),
        );
        if (savedLastOrder)
          setLastOrder(JSON.parse(savedLastOrder) as LastOrderDraft);
      } catch {
  ...
  const isWaiterFlow = Boolean(
    defaultServiceSessionId || defaultWaiterName || defaultCustomerName,
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-600">
            Selecione os itens solicitados. Quando aberto pelo garçom, o
            responsável já vem preenchido.
          </p>
        </div>
        {sessionLoaded && lastOrder ? (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-amber-100 px-4 py-2 text-sm font-black text-green-950"
          >
      ...
      {error === "no-items" ? (
        <div className="mt-4 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-800">
          Selecione pelo menos um item para criar o pedido.
        </div>
      ) : null}
      ...
      {session.waiterName || session.tableLabel || session.customerName ? (
        <div className="mt-4 rounded-3xl border border-amber-100 bg-amber-50 p-4 text-sm text-stone-700">
          <p className="font-black text-green-950">
            Atendimento iniciado pelo garçom
          </p>
          <div className="mt-3 grid gap-3 sm:grid-cols-3">
            <label className="grid gap-1 font-bold">
              ...
              <input
                name="waiter_name"
                value={session.waiterName}
                onChange={(event) =>
                  setSession((current) => ({
                    ...current,
                    waiterName: event.target.value,
                  }))
                }
                className="rounded-2xl border border-amber-100 bg-white p-3 font-normal"
            ...
            <div className="rounded-2xl bg-white p-3">
              ...
              <p className="font-black text-green-950">
                {session.tableLabel || "Mesa não informada"} ·{" "}
                {session.customerName || "Responsável a definir"}
              </p>
            </div>
          </div>
        </div>
      ...
      {isWaiterFlow ? (
        <div className="mt-4 rounded-3xl border border-green-100 bg-green-50 p-4 text-sm text-green-950">
          ...
          <input
            type="hidden"
            name="table_label"
            value={session.tableLabel || session.customerName}
          />
          <p className="font-black">
            Pedido para {session.customerName || "responsável"}
          </p>
          <p className="mt-1 text-stone-700">
            Garçom: {session.waiterName || "não informado"} · Fechamento no
            caixa ao final.
          </p>
        </div>
      ) : (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <label className="grid gap-1 text-sm font-bold">
            ...
            <select
              ...
              className="rounded-2xl border border-stone-200 p-3 font-normal"
            >
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
          {["Todos", ...categories].map((category) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-black ${activeCategory === category ? "bg-green-900 text-white" : "bg-amber-50 text-green-950"}`}
            >
              {category}
            </button>
          ))}
      ...
      <div className="mt-5 grid gap-5">
        {categories.map((category) => {
          ...
          return (
            <section
              ...
              className="scroll-mt-40 rounded-3xl bg-stone-50 p-3"
            >
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {item.requires_preparation ? (
                          <span className="rounded-full bg-amber-50 px-2 py-1 text-[0.65rem] font-black text-amber-900">
                            preparo
                          </span>
                        ) : null}
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <button
                          ...
                          className="flex h-12 w-12 items-center justify-center rounded-full bg-stone-100 text-green-950"
                          aria-label={`Diminuir ${item.name}`}
                        >
                          <Minus className="h-5 w-5" />
                        </button>
                        <span className="min-w-12 text-center text-2xl font-black">
                          {quantity}
                        </span>
                        <button
                          type="button"
                          onClick={() => setQuantity(item.id, quantity + 1)}
                          className="flex h-12 w-12 items-center justify-center rounded-full bg-green-900 text-white"
        ...
        {visibleItems.length === 0 ? (
          <div className="rounded-3xl bg-amber-50 p-5 text-sm font-bold text-amber-900">
      ...
      <label className="mt-4 grid gap-1 text-sm font-bold">
        ...
        <textarea
          ...
          className="min-h-20 rounded-2xl border border-stone-200 p-3 font-normal"
          placeholder="Ex.: sem cebola, entregar na mesa, retirar no balcão..."
        />
      </label>
      ...
      <div className="sticky bottom-3 z-20 mt-5 rounded-[1.5rem] border border-green-100 bg-white p-4 shadow-2xl">
        ...
        <p className="mt-2 flex items-center gap-1 text-xs text-stone-500">
          <UserRound className="h-3 w-3" /> Pedido registrado para conferência
          do garçom/caixa.
        </p>
      </div>
    </form>
