...
import { paidAmountFromOrder, pendingAmountFromOrder, paymentMethodLabel, getConsumptionOrdersForEvent } from "@/lib/operation-dashboard";
...
export async function GET() {
  ...
  for (const order of orders) {
    ...
    const methods = Array.from(new Set(order.payments.filter((payment) => payment.status === "paid").map((payment) => paymentMethodLabel(payment.method)))).join(" + ");
