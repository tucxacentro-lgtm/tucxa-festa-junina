-- 009 - Menu administrativo configurável por evento
-- Esta migration cria o catálogo global de menu e a configuração por evento.
...
create index if not exists admin_menu_items_parent_key_idx on public.admin_menu_items(parent_key);
create index if not exists admin_menu_items_section_sort_idx on public.admin_menu_items(section, sort_order);
create index if not exists event_menu_items_event_idx on public.event_menu_items(event_id);
create index if not exists event_menu_items_item_key_idx on public.event_menu_items(item_key);
...
begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'admin_menu_items' and policyname = 'Service role manages admin menu items'
  ) then
    create policy "Service role manages admin menu items"
    on public.admin_menu_items
    for all
    using (true)
    with check (true);
  end if;
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'event_menu_items' and policyname = 'Service role manages event menu items'
  ) then
    create policy "Service role manages event menu items"
...
insert into public.admin_menu_items (
  item_key, label, description, section, parent_key, route_path, icon_key, sort_order, default_enabled, implemented, active, not_implemented_message
)
values
...
insert into public.event_menu_items (event_id, menu_item_id, item_key, enabled, status, sort_order)
select
  e.id,
  m.id,
  m.item_key,
  case
    when m.item_key in ('vendas_individuais_combos', 'vendas_indicacoes', 'vendas_upsell', 'conveniencias_bingo', 'operacao_pedidos', 'operacao_entrega', 'operacao_caixa', 'operacao_ocorrencias') then false
    else m.default_enabled
  end as enabled,
  case
    when m.item_key in ('vendas_individuais_combos', 'vendas_indicacoes', 'vendas_upsell', 'conveniencias_bingo', 'operacao_pedidos', 'operacao_entrega', 'operacao_caixa', 'operacao_ocorrencias') then 'not_used'
    when m.item_key in ('vendas', 'vendas_convites', 'vendas_acesso', 'vendas_pagamentos', 'painel_evento') then 'in_use'
    when m.item_key in ('conveniencias_cardapio', 'conveniencias_ficha_tecnica', 'operacao_voluntarios', 'operacao_compras') then 'configuring'
    else 'suggested'
  end as status,
  m.sort_order
from public.events e
cross join public.admin_menu_items m
