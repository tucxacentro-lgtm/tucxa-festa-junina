...
set
  ...
  instructions = case method
    ...
    when 'credit' then 'O pagamento por cartão de crédito será realizado presencialmente com a equipe do caixa. Apresente o código/QR Code da compra ou pedido e guarde o comprovante da maquininha até a confirmação pela equipe.'
    when 'debit' then 'O pagamento por débito será realizado presencialmente com a equipe do caixa. Após aprovação na maquininha, a equipe confirmará o pagamento no sistema. Apresente o código/QR Code quando solicitado.'
    when 'cash' then 'O pagamento em dinheiro será realizado presencialmente no caixa. Se possível, leve o valor aproximado para facilitar o troco e agilizar o atendimento. Apresente o código/QR Code quando solicitado.'
